...
from core import OCREngine, TTSEngine, AudioPlayer, TextTranslator, RNNTextNoiseFilter
...
class OCRWorker:
    ...
    def _make_frame_signature(self, frame):
        """Сжать кадр до компактной сигнатуры для быстрого сравнения похожести."""
        ...
        try:
            ...
            h, w = arr.shape[:2]
            target_h = int(getattr(config, "OCR_FRAME_SIGNATURE_HEIGHT", 18))
            target_w = int(getattr(config, "OCR_FRAME_SIGNATURE_WIDTH", 48))
            ...
            sig = arr[np.ix_(ys, xs)].astype(np.float32) / 255.0
            return sig
        except Exception:
            return None

    def _is_similar_frame(self, frame) -> bool:
        """Проверить, похож ли текущий кадр на предыдущий по компактной сигнатуре."""
        if not bool(getattr(config, "OCR_SKIP_SIMILAR_FRAMES", True)):
            return False

        sig = self._make_frame_signature(frame)
        if sig is None:
            return False
        ...
        try:
            ...
            threshold = float(getattr(config, "OCR_FRAME_SIMILARITY_THRESHOLD", 0.012))
    ...
    def run(self):
        ...
        while ThreadLifecycle.is_ocr_running():
            ...
            try:
                ...
                if self._is_similar_frame(screenshot_item):
                    continue
                ...
                if text:
                    ...
                    now = time.time()
                    warn_interval = float(getattr(config, "OCR_NO_TEXT_WARN_INTERVAL_SEC", 5.0))
                    if now - last_no_text_warn_at >= warn_interval:
                        silence_for = now - last_text_at
                        log(
                            f"⚠️ OCR получает кадры, но текста нет уже {silence_for:.1f}s (empty={empty_recognitions})",
                            level="WARNING",
                        )
                        last_no_text_warn_at = now
...
class TextComparisonWorker:
    ...
    def __init__(self, audio_player: AudioPlayer | None = None, translator: TextTranslator | None = None):
        ...
        self.rnn_filter = None
        self._init_rnn_filter()

    def _init_rnn_filter(self):
        """Инициализировать RNN-фильтр OCR-текста (если включен в config)."""
        if not bool(getattr(config, "RNN_TEXT_FILTER_ENABLED", False)):
            return

        try:
            self.rnn_filter = RNNTextNoiseFilter(
                seq_len=int(getattr(config, "RNN_TEXT_FILTER_SEQ_LEN", 48)),
                hidden_size=int(getattr(config, "RNN_TEXT_FILTER_HIDDEN_SIZE", 20)),
                threshold=float(getattr(config, "RNN_TEXT_FILTER_THRESHOLD", 0.55)),
                train_samples=int(getattr(config, "RNN_TEXT_FILTER_TRAIN_SAMPLES", 1200)),
                epochs=int(getattr(config, "RNN_TEXT_FILTER_EPOCHS", 12)),
            )
            log("✅ RNN-фильтр OCR-текста включен", level="INFO")
        except Exception as e:
            self.rnn_filter = None
            log(f"⚠️ RNN-фильтр недоступен, используем fallback-эвристики: {e}", level="WARNING")
    ...
    def _is_meaningful_text(self, text: str) -> bool:
        ...
        if self.rnn_filter is not None:
            try:
                rnn_score = self.rnn_filter.score(text)
                if rnn_score < float(getattr(config, "RNN_TEXT_FILTER_THRESHOLD", 0.55)):
                    return False
            except Exception:
                # Если RNN временно не отработал, не блокируем пайплайн.
                pass
