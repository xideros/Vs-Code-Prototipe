...
class SettingsManager:
    ...
    DEFAULT_SETTINGS = {
        ...
        'game_root': getattr(config, 'GAME_ROOT', ''),
        'auto_detect_game_root': bool(getattr(config, 'GAME_AUTODETECT_ENABLED', True)),
        'ue_extractor_tool': getattr(config, 'UE_EXTRACTOR_TOOL', ''),
        'ue_extractor_output_dir': getattr(config, 'UE_EXTRACTOR_OUTPUT_DIR', ''),
        'ue_extractor_command_template': getattr(
            config,
            'UE_EXTRACTOR_COMMAND_TEMPLATE',
            '"{tool}" "{container}" -Extract "{output_dir}" -Filter="{internal_path}"',
        ),
