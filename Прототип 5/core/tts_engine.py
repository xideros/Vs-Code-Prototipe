...
class TTSEngine:
    ...
    def __init__(self):
        ...
        self.timeout_sec = float(getattr(config, "TTS_SYNTH_TIMEOUT_SEC", 6.0))
    ...
    def synthesize(self, text: str, speed: float = 1.0) -> str:
        ...
        try:
            ...
            try:
                async def _save_with_timeout():
                    communicate = edge_tts.Communicate(
                        text,
                        voice=self.voice,
                        rate=rate_str,
                    )
                    await asyncio.wait_for(communicate.save(audio_path), timeout=self.timeout_sec)
                ...
                if isinstance(e, asyncio.TimeoutError):
                    log(
                        f"❌ Ошибка Edge TTS: timeout after {self.timeout_sec:.1f}s",
                        level="ERROR",
                    )
                else:
                    log(f"❌ Ошибка Edge TTS [{err_name}]: {e!r}", level="ERROR")
