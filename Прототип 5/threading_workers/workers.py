...
from core import TTSEngine, AudioPlayer, TextTranslator
...
class TextComparisonWorker:
    ...
    def _canonical_text(self, text: str) -> str:
        """Привести текст к форме для устойчивого сравнения."""
        if not text:
            return ""
    ...
    def _is_same_or_similar(self, a: str, b: str) -> bool:
        """Считать строки одинаковыми, если они очень похожи."""
    ...
    def _is_meaningful_text(self, text: str) -> bool:
        """Отсечь мусор, который не похож на естественную фразу."""
        canonical = self._canonical_text(text)
        if not canonical:
            return False

        # Для одинаковых языков режем шум по письменности выбранного языка,
        # чтобы корректная реплика не вытеснялась шумным мусором.
...
class AudioPrepWorker:
    ...
    def _prepare_text_for_tts(self, text: str) -> str:
        """Очистить шумный текст и ограничить длину для быстрого TTS."""
        cleaned = (text or "").strip()
        if not cleaned:
            return ""

        # Убираем типичный рекламный мусор и артефакты распознавания/импорта.
        ...
        # Отбрасываем строки с низкой долей букв (часто это шум/артефакты).
    ...
    def synthesize_task(self, payload):
        ...
        try:
            ...
            # Перевод выполняется один раз на этом этапе перед подготовкой TTS.
